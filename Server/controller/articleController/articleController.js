...
const createArticle = async (req, res) => {
    try {
        ...
        // Perform the axios request asynchronously
        const resetRecommendation = axios.get(recommendationEngineUrl + 'reset');

        // Wait for both the article save and the axios request to complete
        await Promise.all([savedArticle, resetRecommendation]);
